...
var FSHADER_SOURCE = `
    ...
    void main(){
        ...
        vec3 lv_Color = texture2D( u_Sampler1, v_TexCoord ).rgb;
        vec3 heart_Color = texture2D( u_Sampler2, v_TexCoord ).rgb;
        if(distance(texColor, vec3(0.0, 0.0, 1.0))<0.6){
          texColor = lv_Color;
        }
        if(distance(texColor, vec3(1.0, 0.0, 0.0))<0.3){
          if(distance(heart_Color, vec3(1.0, 1.0, 1.0))<0.7){
            texColor = texColor*0.4 + heart_Color*0.6;
          }
        }
